...
API_ENV_AUTH_SECRET="$(read_api_env_value AUTH_SECRET || true)"
API_ENV_APP_KEY="$(read_api_env_value APP_KEY || true)"
RUNTIME_AUTH_SECRET="${API_ENV_AUTH_SECRET:-${AUTH_SECRET:-${APP_KEY:-change-me-in-env}}}"
RUNTIME_APP_KEY="${API_ENV_APP_KEY:-${APP_KEY:-${AUTH_SECRET:-change-me-in-env}}}"
...
if [[ -n "${API_ENV_AUTH_SECRET}" ]]; then
  echo "Using auth secret from ${API_DIR}/.env for API and realtime chat"
else
  echo "Using shell/default auth secret for API and realtime chat"
fi
...
(
  ...
  INTERNAL_CHAT_SOCKET_SECRET="${RUNTIME_SOCKET_SECRET}" \
  AUTH_SECRET="${RUNTIME_AUTH_SECRET}" \
  APP_KEY="${RUNTIME_APP_KEY}" \
...
(
  ...
  if [[ "${WEB_MODE}" == "preview" ]]; then
    ...
    VITE_INTERNAL_CHAT_SOCKET_PORT="${REALTIME_PORT}" \
