...
const mapApiStatusToUi = (status: unknown): InvoiceStatus => {
  const normalized = String(status || '').trim().toLowerCase();
  if (normalized === 'cancelled' || normalized === 'canceled') return InvoiceStatus.CANCELLED;
  if (normalized === 'posted') return InvoiceStatus.SENT;
  return InvoiceStatus.DRAFT;
};

const mapApiItem = (item: any, invoiceId: string): InvoiceItem => ({
  id: String(item?.id ?? ''),
...
const mapInvoiceSummary = (raw: any): Invoice => {
  const invoiceId = String(raw?.invoice_refno || raw?.id || '');
  return {
    id: invoiceId,
...
const mapInvoiceDetail = (payload: any): Invoice => {
  const invoice = payload?.invoice || {};
  const items = Array.isArray(payload?.items) ? payload.items : [];
  const summary = payload?.summary || {};
  const mapped = mapInvoiceSummary({
    ...invoice,
...
export const getInvoicesPage = async (filters: InvoicesPageFilters): Promise<InvoicesPageResult> => {
  ...
  const list = await requestApi(`${API_BASE_URL}/invoices?${query.toString()}`);
  const rows = Array.isArray(list?.items) ? list.items : [];
  return {
    items: rows.map(mapInvoiceSummary),
    meta: {
...
export const getInvoice = async (id: string): Promise<Invoice | null> => {
  try {
    const data = await requestApi(
      `${API_BASE_URL}/invoices/${encodeURIComponent(id)}?main_id=${encodeURIComponent(String(API_MAIN_ID))}`
    );
...
const runInvoiceAction = async (id: string, action: string): Promise<Invoice | null> => {
  ...
  const data = await requestApi(`${API_BASE_URL}/invoices/${encodeURIComponent(id)}/actions/${action}`, {
    method: 'POST',
...
export const sendInvoice = async (id: string): Promise<Invoice | null> => runInvoiceAction(id, 'send');

export const markOverdue = async (id: string): Promise<Invoice | null> => runInvoiceAction(id, 'mark_overdue');

export const recordPayment = async (
  id: string,
  _data: { payment_date: string; payment_method: string }
