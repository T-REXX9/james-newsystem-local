...
const mapTransferItem = (raw: any): TransferStockItem => ({
  ...
  from_item_session: String(raw?.from_item_session ?? ''),
  from_warehouse_id: String(raw?.from_warehouse_id ?? ''),
  from_original_qty: toNumber(raw?.from_original_qty, 0),
  to_item_session: String(raw?.to_item_session ?? ''),
  to_warehouse_id: String(raw?.to_warehouse_id ?? ''),
  to_original_qty: toNumber(raw?.to_original_qty, 0),
  transfer_qty: toNumber(raw?.transfer_qty, 0),
  edited: toNumber(raw?.edited, 0),
