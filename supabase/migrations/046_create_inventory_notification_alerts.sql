-- Create server-side inventory alerts for low-stock, out-of-stock, and expiry-aware inventory changes.
...
DECLARE
  ...
  v_title TEXT := NULL;
  v_message TEXT := NULL;
  v_expiry_value TEXT := COALESCE(
    to_jsonb(NEW)->>'expiry_date',
    to_jsonb(NEW)->>'expiration_date',
...
BEGIN
  ...
  IF v_expiry_value IS NOT NULL AND v_expiry_value <> '' AND v_expiry_value::timestamptz <= NOW() THEN
    v_alert_type := 'expired_inventory';
    v_title := 'Expired Inventory Alert';
    v_message := format(
      '%s (%s) is expired and requires review.',
      COALESCE(NEW.description, NEW.item_code, NEW.part_no, 'Unknown Product'),
      COALESCE(NEW.part_no, NEW.item_code, NEW.id)
    );
  ELSIF v_total_stock = 0 AND v_previous_total_stock <> 0 THEN
...
DROP TRIGGER IF EXISTS trigger_notify_inventory_threshold_change ON public.products;
CREATE TRIGGER trigger_notify_inventory_threshold_change
AFTER UPDATE
ON public.products
FOR EACH ROW
EXECUTE FUNCTION public.notify_inventory_threshold_change();
