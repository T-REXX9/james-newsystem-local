...
const OperationsDashboard: React.FC<OperationsDashboardProps> = ({ onNavigate }) => {
  ...
  const [callBreakdown, setCallBreakdown] = useState<OperationsCallBreakdownKind | null>(null);
  ...
  const selectedPeriod = useMemo(() => {
    ...
    const monthEndValue = `${year}-${month}-${String(monthEnd.getDate()).padStart(2, '0')}`;
    return {
      dashboardDate: selectedDate,
      dashboardMonth: month,
      dashboardYear: year,
      dashboardMonthStart: monthStart,
      dashboardMonthEnd: monthEndValue,
    };
  }, [selectedDate]);
  const nav = (route: string, payload?: Record<string, string>) => () => onNavigate(route, { ...selectedPeriod, ...(payload || {}) });
  const orderCards = [
  ...
  return <div className="h-full overflow-y-auto bg-[#f7f8fb] p-3 text-slate-900 sm:p-5 xl:p-6"><div className="mx-auto max-w-[1700px] space-y-4">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <Section title="2. Call Overview (Selected Date)"><MetricRow label="Incoming Calls" value={snapshot.calls.incoming} icon={PhoneIncoming} onClick={() => setCallBreakdown('incoming')} /><MetricRow label="Outgoing Calls" value={snapshot.calls.outgoing} icon={PhoneOutgoing} tone="cyan" onClick={() => setCallBreakdown('outgoing')} /><MetricRow label="Missed Calls" value={snapshot.calls.missed} icon={PhoneMissed} tone="red" onClick={() => setCallBreakdown('missed')} /><MetricRow label="Returned Calls" value={snapshot.calls.returned} icon={PhoneCall} tone="green" onClick={() => setCallBreakdown('returned')} /><MetricRow label="Unanswered Calls" value={snapshot.calls.unanswered} icon={Phone} tone="orange" onClick={() => setCallBreakdown('unanswered')} /><div className="mt-3 rounded-lg bg-blue-50 px-3 py-2 text-right"><span className="float-left text-[10px] font-bold uppercase text-slate-500">Average Duration</span><strong className="text-lg text-blue-700">{duration(snapshot.calls.averageResponseSeconds)}</strong></div></Section>
      <Section title="3. Delivery Overview (This Month)"><MetricRow label="Ready to Ship" value={snapshot.delivery.ready} icon={Truck} onClick={nav('sales-transaction-order-slip', { dashboardSlipStatus: 'draft' })} /><MetricRow label="Shipped" value={snapshot.delivery.shipped} icon={Truck} tone="cyan" onClick={nav('sales-transaction-order-slip', { dashboardSlipStatus: 'finalized' })} /><MetricRow label="In Transit" value={snapshot.delivery.inTransit} icon={Truck} tone="green" onClick={nav('sales-transaction-order-slip')} /><MetricRow label="Delivered" value={snapshot.delivery.delivered} icon={PackageCheck} tone="green" onClick={nav('sales-transaction-order-slip')} /><MetricRow label="Delayed Delivery" value={snapshot.delivery.delayed} icon={Clock3} tone="orange" onClick={nav('sales-transaction-order-slip')} /><MetricRow label="Failed Delivery" value={snapshot.delivery.failed} icon={AlertCircle} tone="red" onClick={nav('sales-transaction-order-slip', { dashboardSlipStatus: 'cancelled' })} /><p className="mt-3 flex justify-between border-t pt-2 text-xs font-bold"><span>Total Shipments</span><strong className="text-xl text-blue-700">{snapshot.delivery.total}</strong></p></Section>
      ...
      <Section title="5. Sales Return Overview (This Month)"><MetricRow label="Return Requests" value={snapshot.returns.requests} icon={ClipboardCheck} tone="violet" onClick={nav('accounting-transactions-sales-return-credit', { dashboardReturnStatus: 'All' })} /><MetricRow label="Under Inspection" value={snapshot.returns.inspection} icon={ClipboardCheck} tone="violet" onClick={nav('accounting-transactions-sales-return-credit', { dashboardReturnStatus: 'Pending' })} /><MetricRow label="Approved" value={snapshot.returns.approved} icon={CheckCircle2} tone="green" onClick={nav('accounting-transactions-sales-return-credit', { dashboardReturnStatus: 'Posted' })} /><MetricRow label="Disapproved" value={snapshot.returns.disapproved} icon={XCircle} tone="red" onClick={nav('accounting-transactions-sales-return-credit', { dashboardReturnStatus: 'Canceled' })} /><MetricRow label="Replacement Sent" value={snapshot.returns.replacement} icon={FolderOpen} onClick={nav('accounting-transactions-sales-return-credit', { dashboardReturnStatus: 'All' })} /><MetricRow label="Refund Completed" value={snapshot.returns.refunded} icon={RotateCcw} tone="violet" onClick={nav('accounting-transactions-sales-return-credit', { dashboardReturnStatus: 'All' })} /><p className="mt-3 flex justify-between border-t pt-2 text-xs font-bold"><span>Approval Rate</span><strong className="text-lg text-violet-700">{percent(snapshot.returns.requests ? snapshot.returns.approved / snapshot.returns.requests * 100 : 0)}</strong></p></Section>
    </div>
    ...
    {callBreakdown && <OperationsCallBreakdownModal date={selectedDate} details={snapshot.callDetails || []} kind={callBreakdown} onClose={() => setCallBreakdown(null)} />}
  </div></div>;
};
