          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
            ...
            <tbody className="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-900/30">
              {filteredRecords.map((record) => (
                <tr key={record.id}>
                  <td className="px-3 py-2 text-slate-700 dark:text-slate-200">{new Date(record.date).toLocaleDateString('en-PH', { month: 'short', day: '2-digit', year: '2-digit' }).replace(/ /g, '\u2011').replace(',', '').toUpperCase()}</td>
