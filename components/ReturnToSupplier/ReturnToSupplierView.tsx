        ...
        <section className="w-full rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="p-6">
                ...
                <div className="mt-8 grid grid-cols-[100px_1fr] items-center gap-4 text-sm">
                    ...
                    <span className="font-bold text-slate-700">{new Date(returnRecord.return_date).toLocaleDateString('en-PH', { month: 'short', day: '2-digit', year: '2-digit' }).replace(/ /g, '\u2011').replace(',', '').toUpperCase()}</span>
                </div>
            </div>
