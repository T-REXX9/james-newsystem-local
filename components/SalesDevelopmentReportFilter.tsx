        ...
        <div className="flex items-center gap-4 mb-8 animate-slideInUp">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">
              Identify missed opportunities and plan inventory demand
            </p>
          </div>
        </div>
          ...
          <div className="p-8 space-y-10">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button
                  onClick={() => setReportCategory('not_purchase')}
                  className={`relative overflow-hidden p-6 rounded-xl text-left transition-all duration-300 ${reportCategory === 'not_purchase'
                    ? 'bg-brand-blue text-white shadow-lg shadow-brand-blue/30 ring-2 ring-brand-blue ring-offset-2 dark:ring-offset-slate-900'
                    : 'bg-slate-100 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'
                    }`}
                >
                  ...
                  <h3 className={`font-bold mb-1 ${reportCategory === 'not_purchase' ? 'text-white' : ''}`}>Not Purchased (Lost Sales)</h3>
                  <p className={`text-sm ${reportCategory === 'not_purchase' ? 'text-white/80' : 'text-slate-500 dark:text-slate-400'}`}>
                    Items marked "On Stock" but no sales order generated
                  </p>
                </button>
                <button
                  onClick={() => setReportCategory('no_stock')}
                  className={`relative overflow-hidden p-6 rounded-xl text-left transition-all duration-300 ${reportCategory === 'no_stock'
                    ? 'bg-brand-blue text-white shadow-lg shadow-brand-blue/30 ring-2 ring-brand-blue ring-offset-2 dark:ring-offset-slate-900'
                    : 'bg-slate-100 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'
                    }`}
                >
                  {reportCategory === 'no_stock' && (
                    <span className="absolute inset-0 bg-gradient-to-tr from-white/20 to-transparent opacity-50"></span>
                  )}
                  <h3 className={`font-bold mb-1 ${reportCategory === 'no_stock' ? 'text-white' : ''}`}>No Stock (Unmet Demand)</h3>
                  <p className={`text-sm ${reportCategory === 'no_stock' ? 'text-white/80' : 'text-slate-500 dark:text-slate-400'}`}>
...
export default SalesDevelopmentReportFilterView;
