...
const InvoiceView: React.FC<InvoiceViewProps> = ({ initialInvoiceId, initialInvoiceRefNo }) => {
  ...
  const selectInvoiceAndSync = useCallback((invoice: Invoice) => {
    void selectInvoice(invoice);
    navigateWorkflow(INVOICE_TAB_ID, { invoiceId: invoice.id, invoiceRefNo: invoice.invoice_no }, 'replace');
  }, [selectInvoice]);
  ...
  const handleUnpost = async () => {
    ...
    try {
      ...
      await unpostInvoice(selectedInvoice.id);
      const salesOwnerUserId = await resolveNotificationUserId(selectedInvoice.created_by, selectedInvoice.sales_person);
      await notifyInvoiceEvent(
        'Invoice Unposted',
  ...
  }, [addToast, jpegCaptureMode, selectedInvoice?.invoice_no]);
      ...
      <div className="mx-auto w-full max-w-[1680px] space-y-[26px]">
        ...
        <section className="min-h-[517px] overflow-hidden rounded-[5px] border border-[#d7d7d7] bg-white">
          <div className="flex h-[64px] items-center justify-between border-b border-[#d7d7d7] px-5">
            <div className="relative flex h-full items-center text-[18px] font-semibold text-[#29475f] after:absolute after:bottom-[-1px] after:left-0 after:h-px after:w-[89px] after:bg-[#6a92b3]">INVOICE</div>
            <button
              type="button"
