                                    ...
                                    <div key={tx.id || i} className="flex items-center gap-4 py-3 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 -mx-5 px-5 transition-colors cursor-pointer">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="text-xs text-slate-400">{new Date(tx.date).toLocaleDateString('en-PH', { month: 'short', day: '2-digit', year: '2-digit' }).replace(/ /g, '\u2011').replace(',', '').toUpperCase()}</span>
                                            </div>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {transactions.filter(t => ['invoice', 'order_slip', 'sales_order'].includes(t.type)).map((tx) => (
                                    <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                        <td className="p-4 text-sm text-slate-600 dark:text-slate-400">{new Date(tx.date).toLocaleDateString('en-PH', { month: 'short', day: '2-digit', year: '2-digit' }).replace(/ /g, '\u2011').replace(',', '').toUpperCase()}</td>
                                        <td className="p-4">
                                ...
                                <div key={tx.id} className="flex justify-between items-center p-3 border border-slate-200 rounded-lg bg-white">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs text-slate-400">Due: {new Date(tx.date).toLocaleDateString('en-PH', { month: 'short', day: '2-digit', year: '2-digit' }).replace(/ /g, '\u2011').replace(',', '').toUpperCase()}</div>
                                        </div>
                                    </div>
                                    <div className="text-right">
