...
import { ArrowLeft, CalendarDays, CheckCircle2, MessageSquare, Plus, Send, Trash2, X } from 'lucide-react';
...
const PurchaseRequestForm: React.FC<PurchaseRequestFormProps> = ({ onCancel, onSubmit, suppliers, initialPRNumber }) => {
  ...
  const buildItem = (): DraftItem | null => {
    ...
    const resolvedQuantity = quantity === '' ? 0 : quantity;
    const quantityValidation = validateNumeric(resolvedQuantity, 'quantity', 1);
        ...
        <main className="mx-auto max-w-[1500px] p-5 lg:p-8">
          ...
          <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              <div><h2 className="text-base font-extrabold uppercase tracking-wide text-[#173c83]">Items for Purchase Request</h2></div>
            </div>
                    ...
                    <td colSpan={3} className="px-3 py-3">
                      ...
                      {selectedProduct ? (
                        <div className="mt-1.5 flex items-center gap-2 rounded border border-blue-200 bg-blue-50 px-2.5 py-1.5">
                          ...
                          <span className="text-xs font-medium text-slate-700 truncate">{selectedProduct.description || '-'}</span>
                        </div>
                      ) : null}
                    </td>
                    <td className="px-3 py-3">
                      <div className="relative flex items-center">
                        <input
                          ...
                          className="h-9 w-full rounded border border-slate-300 bg-white pl-2 pr-8 text-xs text-slate-700 outline-none focus:border-[#175fd3] focus:ring-2 focus:ring-blue-100 [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:right-0 [&::-webkit-calendar-picker-indicator]:w-8 [&::-webkit-calendar-picker-indicator]:h-full [&::-webkit-calendar-picker-indicator]:cursor-pointer"
                        />
                        <CalendarDays className="pointer-events-none absolute right-2 h-3.5 w-3.5 text-slate-400" />
                      </div>
                    </td>
