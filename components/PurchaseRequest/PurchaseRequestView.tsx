...
const PurchaseRequestView: React.FC<PurchaseRequestViewProps> = ({ request, onBack, onUpdate, onUpdateItem, onDeleteItem, onAddItem, onConvert, onPrint, products, suppliers, isApprover = true }) => {
  ...
  const generatedPOs = Array.from(new Set(items.map((item: any) => item.po_number).filter(Boolean)));
      ...
      <div className="mx-auto max-w-[1500px] space-y-5 p-5 lg:p-8">
        ...
        {generatedPOs.length > 0 && (
          <section className="rounded-lg border border-amber-200 bg-amber-50 p-5 shadow-sm flex items-start gap-3">
            ...
            <div>
              <h3 className="font-bold text-amber-800">Purchase Order Generated</h3>
              <p className="text-sm text-amber-700 mt-1">
                This Purchase Requisition is linked to the following Purchase Order(s): <strong>{generatedPOs.join(', ')}</strong>.<br/>
